...
1) CentOS(7.2) installation:
    After completing the below installation steps for CentOS, skip the Ubuntu
    installation step #2 below and go to step #3
    ...
    c) Install a newer qemu version
       The qemu package should be qemu-img-ev-2.3.0-31.el7.16.1.x86_64
       or newer
    ...
    d) Install the etcd distributed key value store
...
2) Ubuntu(16.04) installation:
    ...
    c) Install the etcd distributed key value store
...
5) Steps for devstack
   ...
   ::
     ...
     #For a multi-node deployment make sure to use the correct uplink interface on each compute node
