...
class VPPForwarderTestCase(base.BaseTestCase):
    ...
    @mock.patch(
        'networking_vpp.agent.server.VPPForwarder.ensure_network_on_host')
    def _test_create_router_on_host(self, m_network_on_host, ip_ver):
        if ip_ver == 6:
    ...
    @mock.patch(
        'networking_vpp.agent.server.VPPForwarder.ensure_network_on_host')
    def _test_router_create_with_existing_bvi(self, m_network_on_host, ip_ver):
        if ip_ver == 6:
            router = self._get_mock_v6_router()
        else:
            router = self._get_mock_router()
    ...
    def test_v4_router_create_on_host(self):
        self._test_create_router_on_host(ip_ver=4)

    def test_v6_router_create_on_host(self):
        self._test_create_router_on_host(ip_ver=6)

    def test_v4_router_create_with_existing_bvi(self):
        self._test_router_create_with_existing_bvi(ip_ver=4)

    def test_v6_router_create_with_existing_bvi(self):
        self._test_router_create_with_existing_bvi(ip_ver=6)
