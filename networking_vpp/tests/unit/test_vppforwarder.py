...
class VPPForwarderTestCase(base.BaseTestCase):
    ...
    def test_ensure_bridge_found(self, m_br_lib):
        self.vpp.ensure_bridge('test_br')
        assert m_br_lib.BridgeDevice.called_once_with('test_br')
    ...
    def test_ensure_bridge(self, m_br_ex, m_br_lib):
        ...
        self.vpp.ensure_bridge('test_br')
        assert m_br_lib.BridgeDevice.addbr.called_once_with('test_br')
    ...
    @mock.patch('networking_vpp.agent.server.Thread')
    def test_create_interface_on_host_plugtap(self, m_thread, m_en_br):
        if_type = 'plugtap'
        uuid = 'fakeuuid'
        mac = 'fakemac'
        retval = self.vpp.create_interface_on_host(if_type, uuid, mac)
    ...
    #(schander) re-enable this test once there's a more specific Exception
    # in code
