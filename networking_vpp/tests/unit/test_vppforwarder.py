...
class VPPForwarderTestCase(base.BaseTestCase):
    ...
    @mock.patch(
        'networking_vpp.agent.server.VPPForwarder.ensure_network_on_host')
    def _test_router_create_with_existing_bvi_and_ip(self, m_network_on_host,
                                                     router):
        m_network_on_host.return_value = {'bridge_domain_id': 'fake_dom_id'}
    ...
    @mock.patch(
        'networking_vpp.agent.server.VPPForwarder.ensure_network_on_host')
    def _test_router_create_with_existing_bvi_different_ip(
    ...
    def test_v4_router_create_with_existing_bvi_and_ip(self):
        self._test_router_create_with_existing_bvi_and_ip(
            router=self._get_mock_router())

    def test_v6_router_create_with_existing_bvi_and_ip(self):
        self._test_router_create_with_existing_bvi_and_ip(
            router=self._get_mock_v6_router())

    def test_v4_router_create_with_existing_bvi_different_ip(self):
        self._test_router_create_with_existing_bvi_different_ip(
            router=self._get_mock_router(),
            other_router=self._get_mock_v6_router())

    def test_v6_router_create_with_existing_bvi_different_ip(self):
        self._test_router_create_with_existing_bvi_different_ip(
            router=self._get_mock_v6_router(),
    ...
    def test_create_floatingip_on_vpp(self, m_network_on_host):
        ...
        self.vpp.vpp.set_snap_static_mapping.assert_called_once_with(
