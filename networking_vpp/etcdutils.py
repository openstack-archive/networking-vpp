...
class EtcdWatcher(object):
    ...
    def __init__(self, etcd_client, name, watch_path, election_path=None,
        ...
        # NB: heartbeat + 5 + DANGER_PAUSE + whatever work you do is
        ...
        if wait_until_elected:
            # TODO(ijw) will recover a leader after recovery_time - if
            # work takes longer than recovery_time, this can cause another
            # thread to activate.  Make more logical.
            election_time = heartbeat + 5 + DANGER_PAUSE + self.recovery_time

            self.etcd_watcher = EtcdWatcher(etcd_client, name, election_path,
                                            thread_id,
                                            recovery_time=election_time)
