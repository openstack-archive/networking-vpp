...
class EtcdClientSecured(etcd.client.Client):
    ...
    def _check_etcdRes(self, rv):
        for f in rv.children:
            ...
            self.jwt_agent.verify_jwt(signerNodeName,
                                      f.key,
                                      f.value)
    ...
    def write(self, key, value, ttl=None, dir=False,
        ...
        LOG.debug(("EtcdClientSecured write0 :%s;%s") % (key, value))
        value = self.jwt_agent.add_jwt(key, value)
        LOG.debug(("EtcdClientSecured write1 :%s;%s") % (key, value))
    ...
    def read(self, key, **kwdargs):
        LOG.debug(("EtcdClientSecured read0 :%s") % (key))
        ret = super(EtcdClientSecured, self).read(key, **kwdargs)
        LOG.debug(("EtcdClientSecured read1 :%s;%s") % (key, ret))
