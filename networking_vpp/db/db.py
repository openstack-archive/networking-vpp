            ...
            try:
                ...
                if len(rs) > 0:
                    ...
                    try:
                        ...
                        # For some reason, we can't do the job.  This is uncommon.
                        LOG.warning("Couldn't forward etcd record %d due to"
                                    " exception %s. retrying later",
                                    first_id, type(e).__name__)

                        try:
                            # The retry count and time are niceties for admin debugging.
                            # The update can itself fail (DB connection problems, etc.)
                            # but since it's a nicety it's not critical.
                            entry.retry_count += 1
