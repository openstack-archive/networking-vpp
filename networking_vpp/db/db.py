...
def journal_read(session, func):
    ...
    if entry:
        LOG.debug('forward worker found entry')
        ...
        with session.begin(subtransactions=True):
            ...
            if len(rs) > 0:  # The entry is still around, we are still master
                ...
                if func(entry.k, entry.v):  # Lets work on it, but can fail too
                    ...
                    LOG.debug('forward worker job complete. deleted entry')
                else:
                ...
                maybe_more = False
                LOG.debug('forward worker entry went away!')

    else:
        # The table is empty - no work available.
        LOG.debug('forward worker found nothing to do')
        maybe_more = False
