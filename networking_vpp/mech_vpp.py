...
class VPPMechanismDriver(api.MechanismDriver):
    ...
    def initialize(self):
        ...
        self.port_management = FeaturePortBinding()
        self.physnets = FeaturePhysnets()
        self.communicator.register_feature(self.keepalive)
        self.communicator.register_feature(self.port_management)
    ...
    def physnet_known(self, host, physnet):
        physnets = self.physnets.read()
        LOG.debug("physnet %s in physnets %s: %s" % (str((host, physnet)),
                                                     str(physnets),
                                                     str(((host, physnet)
                                                         in physnets))))
        return (host, physnet) in physnets
        ...
        if port_context.binding_levels is not None:
            ...
            if (current_bind is not None and
               current_bind.get(api.BOUND_DRIVER) == self.MECH_NAME):
                ...
                # port)
                self.port_management.bind(port_context._plugin_context.session,
                ...
                self.port_management.unbind(port_context._plugin_context.session,
                                            port_context.current,
...
class FeaturePhysnets(Feature):
    ...
    def read(self):
        ...
        for rv in self.communicator.etcd_client.read(LEADIN, recursive=True).children:
...
class FeaturePortBinding(Feature):
    ...
    def _port_path(self, host, port):
        return self.communicator.port_key_space + "/" + host + "/ports/" + port['id']
...
class EtcdAgentCommunicator(object):
    ...
    def register_feature(self, feature):
        if feature.path in self.features.keys():
            raise Exception('Feature Registration for "%s" can not be done twice'
                            % feature.path)
        ...
        while True:
            ...
            try:
				...
				kv.action = 'create'
                            ...
                            LOG.warn('method: %s for key %s' % (method, kv.key))
                            feature_instance = self.features[feature]
                            getattr(feature_instance, method)(host, key, kv.value)
