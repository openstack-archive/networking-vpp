...
def mac_to_bytes(mac):
    # py3 note:
    # TODO(onong): watch out for the following PAPI change which might render
    # this redundant:
    #
    # "A new vl_api_mac_address_t, which I will create a macaddress object for,
    # so no need to manually decode and encode between text representation of
    # mac addresses"
    if six.PY2:
...
def fix_string(s):
    ...
    # this redundant:
    #
    # "a string type for printable strings, so no longer the need for the
    # funny chopping off of 0's at the end"
    if six.PY2:
        ...
        if type(s) == bytes:
            ...
            return s.rstrip("\0").decode('utf-8')
