...
class VPPInterface(object):
    ...
    def create_tap(self, ifname, mac):
        ...
        t = self._vpp.tap_connect(use_random_mac=False,  # random MAC
                                 tap_name=str(ifname),
                                 mac_address=mac_to_bytes(mac),
                                 renumber=False,  # renumber - who knows, no doc
                                 custom_dev_instance=0) # customdevinstance - who knows, no doc
    ...
    def create_vhostuser(self, ifpath, mac,
        ...
        t = self._vpp.create_vhost_user_if(is_server=is_server,
                                          sock_filename=str(ifpath),  # unicode not allowed.
                                          renumber=False,  # Who knows what renumber is?
                                          custom_dev_instance=0,  # custom_dev_instance
                                          use_custom_mac=True,  # use custom MAC
                                          mac_address=mac_to_bytes(mac)
                                          )
