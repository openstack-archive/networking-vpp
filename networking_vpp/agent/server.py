    ...
    def vhost_notify_thread(self):
        ...
        while True:
            ...
            with open('/proc/net/unix') as file:
                # Track unix sockets in our directory that are open more than once
                for f in file:
                    _, file = f.rsplit(' ', 1) # Problems with files with spaces in, though
                    if file.startswith(dirname): # should be configurable
            ...
            # Update this *before* making callbacks so that these register as up
