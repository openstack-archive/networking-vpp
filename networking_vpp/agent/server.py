...
class EtcdListener(object):
    def __init__(self, host, etcd_client, vppf, physnets):
        ...
        self.iface_state = {}
        # key: sw if index in VPP; present when vhost-user is connected
        self.iface_connected = set()
        cb_event = self.vppf.vpp.CallbackEvents.VHOST_USER_CONNECT
        self.vppf.vpp.register_for_events(cb_event, self._vhost_ready_event)
    ...
    def bind(self, id, binding_type, mac_address, physnet, network_type,
        ...
        if props['iface_idx'] in self.iface_connected:
