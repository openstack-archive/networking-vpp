...
class VPPForwarder(object):
    ...
    def acl_add_replace_on_host(self, secgroup):
        ...
        self.deferred_delete_acls.discard(secgroup.id)
    ...
    def vpp_restart_prepare(self):
        ...
        for f in self.vppf.get_interfaces():
            ...
            if uplink_data is not None:
                ...
                uplink_ports_configured.append([
                    net_type, seg_id,
                    f['sw_if_index'],
                    f['sw_if_index'] if f['sup_sw_if_idx'] is None
        ...
        def clean_up_physnet_link(physnet_name, if_index, port_list):
            # We need to clean up this physnet, its
            # subinterfaces and their bridge domains.
            uplinks = [x for x in port_list
                       if x[3] == if_index]
            ...
            return [x for x in port_list if x[3] != if_index]
        ...
        for name, if_name in self.physnets.items():
            ...
            if if_index != physnet_ports_configured[name]:
                ...
                uplink_ports_configured = \
                    clean_up_physnet_link(if_index, uplink_ports_configured)
        ...
        for net_type, seg_id, sw_if_index, sup_sw_if_idx \
                in physnet_ports_configured:
        ...
        # interfaces in the right bridges.  and move those in the
        # wrong bridges.  Dead bridges have been deleted and binding
        # will find a new home for the interfaces that still exist.

        # At present, there's no such thing as a 'bad' port, since
        # it's either created and tagged (in which case we found it)
        # or not created.  It may be outside a bridge at this point,
        # but that's fine.  Report all ports so that the resync and
        # resync_end will deal with them.
        return bound_ports
