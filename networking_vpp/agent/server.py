...
# ################## BEGIN LISP GPE Methods #################################
        ...
        if (mac, vni) not in self.gpe_map['remote_map']:
            ...
            if ip_network(unicode(ip)).version == 4:
                ...
                self.vpp.add_lisp_arp_entry(mac,
                                            bridge_domain,
                                            int(ip_address(unicode(ip)))
                                            )

    def delete_remote_gpe_mapping(self, vni, mac, ip):
    ...
    def clear_remote_gpe_mappings(self, segmentation_id):
        ...
        LOG.debug("Clearing all gpe remote mappings for VNI:%s",
                  segmentation_id)
        # TODO(najoy): Fetch existing ARP entries for the remote IPv4 IPs in
        # the BD and clear them. There is a VPP papi issue in 17.07-rc1
        # in fetching ARP entries in a BD.
...
class EtcdListener(object):
    ...
    def acl_add_replace(self, secgroup, data):
        ...
        def _secgroup_rule(r):
            # These changes will go away when rebased with remote-group-id
            # changes. These are required temporarily in this patch for
            # compatiblity with the new security-group etcd model for
            # remote-group-id
