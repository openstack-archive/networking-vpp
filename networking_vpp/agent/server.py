...
# enable gpe only if the vxlan type driver is present
enable_gpe = 'vxlan' in cfg.CONF.ml2.type_drivers
...
class VPPForwarder(object):
    ...
    def __init__(self,
        ...
        # Enable the VXLAN-GPE service
        if enable_gpe:
            self.gpe = gpe.GPE(self)
    ...
    def ensure_network_in_vpp(self, physnet, net_type, seg_id):
        ...
        if net_type == 'flat':
            ...
            bridge_idx = self.gpe.bridge_idx_for_lisp_segment(seg_id)
...
class EtcdListener(object):
    def __init__(self, host, client_factory, vppf, physnets):
        ...
        if enable_gpe:
            ...
            self.gpe_key_space = self.gpe_listener.gpe_key_space
    ...
    def bind(self, bound_callback, id, binding_type, mac_address, physnet,
        ...
        if network_type == 'vxlan':
            ...
            physnet = self.vppf.gpe.gpe_locators
    ...
    def process_ops(self):
        ...
        if enable_gpe:
            ...
            self.pool.spawn(gpe.GpeWatcher(self.client_factory.client(),
                                           'gpe_watcher',
                                           self.gpe_key_space,
                                           heartbeat=self.AGENT_HEARTBEAT,
                                           data=self).watch_forever)
