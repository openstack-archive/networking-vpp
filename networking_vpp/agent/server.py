...
class VPPForwarder(object):
    ...
    def delete_interfaces(self, interfaces):
        ...
        params:
        - interfaces: list of (tag, if_idx) of interfaces we have to remove
        """
        for (tag, if_idx) in interfaces:
            if self.vpp.is_vhostuser(if_idx):
                self.vpp.ifdown(if_idx)
                for acl_index in self.vpp.get_acl_interface_list_dump(if_idx):
                    self.vpp.acl_delete(acl_index)
