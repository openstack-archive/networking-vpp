...
class EtcdListener(object):
    ...
    def bind(self, etcd_client, id, binding_type, mac_address, physnet, network_type,
             segmentation_id):
        ...
        if (binding_type != 'vhostuser' or
           self.vppf.vhostuser_linked_up(iface_idx)):
            ...
            self._mark_up(etcd_client, iface_idx)
    ...
    def _mark_up(self, etcd_client, sw_if_index):
        ...
        etcd_client.write(self.state_key_space + '/%s' % port,
                          json.dumps(props))
	...
	etcd_client = self.client_factory.client())
        ...
        for f in physnets:
            etcd_client.write(LEADIN + '/state/%s/physnets/%s'
                              % (self.host, f), 1)
        ...
        self.pool.spawn(PortWatcher(self.client_factory.client(), 'port_watcher',
                                    self.port_key_space,
                                    heartbeat=self.AGENT_HEARTBEAT,
                                    data=self).watch_forever)
