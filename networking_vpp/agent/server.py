...
# #################### LISP GPE Methods ###############################
    ...
    def delete_gpe_remote_mapping(self, segmentation_id, mac_address):
        ...
        for result in get_child_keys().children:
            # TODO(najoy): Fix the type of result. It should be a
            # ParsedEtcdResult that works with signed keys instead
            # of the EtcdResult, and json.loads would not be required,
            # but there's some issue in etcdutils that causes an issue.
