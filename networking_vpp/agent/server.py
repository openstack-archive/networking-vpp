...
class VPPForwarder(object):
    ...
    def __init__(self,
        ...
        # Remote-group UUID to the set to security-groups that reference it
        ...
        for secgroup in self.deferred_delete_secgroups:
            ...
            try:
                ...
                if used:
                    ...
                    for remote_group in self.remote_group_secgroups.keys():
        ...
        def _secgroup_rule(r):
            ...
            # Create a rule for each ip address in the remote_group
                ...
                remote_ip_prefixes = [
                    ...
                    self.vppf.remote_group_ports.get(remote_group, set())
                    for ip in self.vppf.port_ips.get(port, set())
...
# When ports associated with a remote-group are updated, this thread updates
# the security-group ACLs for those security-groups that refer to these
# remote-groups. This thread also keeps tracks of all the ports within a
# remote-group and their IP addresses.
