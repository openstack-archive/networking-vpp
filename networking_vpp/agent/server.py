...
class VPPForwarder(object):
    ...
    def clean_interface_from_vpp(self, uuid, props):
        ...
        if props['bind_type'] == 'vhostuser':
            ...
            # TODO(najoy): Find a way to delete a vhostuser sub-int from VPP.
            # Currently when a single vhostuser sub-intf is deleted, all
            # vhostuser sub-interfaces and the primary vhostuser interface
            # is deleted from VPP.
...
class PortWatcher(etcdutils.EtcdChangeWatcher):
    ...
    def do_tick(self):
        ...
        try:
            ...
            # refresh method.
            # TODO(onong): should be doing a compare-and-swap with our
            # identity as this would prevent two agents running.
            self.etcd_client.write(LEADIN + '/state/%s/alive' %
                                   self.data.host, 1,
            ...
            #   - etcd unreachable/network issues, or
            #   - a very old/errant etcd version with no refresh support
            LOG.exception("Failed to refresh 'alive' key", e)
            LOG.critical("Is another client active? Is etcd unreachable?"
                         " Exiting...")
            sys.exit(1)
