...
class VPPForwarder(object):
    ...
    def _get_snat_indexes(self, floatingip_dict):
        ...
        if internal_network_data:
            ...
            if floatingip_dict['external_net_type'] == p_const.TYPE_VLAN:
    ...
    def ensure_router_interface_on_host(self, port_id, router_data):
        ...
        if loopback_idx not in int_list and enable_snat:
            ...
            # Set the SNAT address to the gateway's external address
            if not is_inside:
                self.vpp.add_del_snat_address(
                    self._pack_address(gateway_ip),
                    vrf)

        # Add VXLAN GPE mappings for vxlan type networks
        if net_type == p_const.TYPE_VXLAN:
        ...
        if is_inside:
            ...
            # Ensure that the gateway network is exported into the tenant VRF
            # self.update_external_route_from_tenant_vrfs(router_dict['vrf_id'])
        else:
            LOG.debug("Router: Created outside router port: %s",
                      router_dict)
            self.router_external_interfaces[port_id] = router_dict
            # Ensure that the gateway network is exported into the tenant VRF
            # self.update_external_route_from_tenant_vrfs()
    ...
    def update_external_route_from_tenant_vrfs(self, source_vrf=0,
                                               is_add=True):
        ...
        """
        if source_vrf:
            LOG.debug("Router: Updating external route from tenant VRF:%s",
                      source_vrf)
        else:
            LOG.debug("Router: Updating external route from all tenant VRFs")
        for ext_port in self.router_external_interfaces:
            ...
            for int_port in self.router_interfaces.values():
                ...
                if is_add:
                    ...
                    # to enable the traffic to flow out. Exporting is done by
                    # setting the next hop sw if index to the gw_port
                    self.vpp.add_ip_route(
                        vrf=gw_port['vrf_id'],
    ...
    def delete_router_interface_on_host(self, port_id):
        ...
        SNAT address for an external interface.
        """
        ...
        # Delete the external network SNAT and default routes in all VRFs
        ...
        if not router['is_inside']:
            ...
            if router['gateway_ip'] in addrs:
                self.vpp.add_del_snat_address(
                    self._pack_address(router['gateway_ip']),
                    router['vrf_id'],
                    is_add=False)

            self.update_external_route_from_tenant_vrfs(source_vrf=0,
                                                        is_add=False)
        else:
            # Delete the default route from the tenant VRF for the internal
            # router interface
            self.update_external_route_from_tenant_vrfs(
                source_vrf=router['vrf_id'],
                is_add=False)
