...
class VPPForwarder(object):
    ...
    def bind_subport_on_host(self, parent_port, subport_data):
        ...
        # parent port must be already bound.
        ...
        # is the subport interface already in vpp?
        ...
        # VPP crashes if a parent port is unbound before its subports
        # Mark the parent as a trunk to prevent early unbinding
...
# #################### LISP GPE Methods ###############################
    ...
    def reconsider_trunk_subports(self):
        ...
        for parent_port in self.subports_awaiting_parents:
            subports = self.subports_awaiting_parents[parent_port]
            ...
            # Parent is bound, so bind the subport
            if props:
    ...
    def subports_to_unbind(self, parent_port, subports):
        ...
        subports :- set of subports of the parent port.
        """
    ...
    def subports_to_bind(self, parent_port, subports):
        """Return a list of subports to unbind for a parent port."""
        # remove ports from subports that are already bound
    ...
    def bind_unbind_subports(self, parent_port, subports):
        ...
        LOG.debug('Binding or Unbinding subports %s of parent port %s',
                  subports, parent_port)
        ...
        for subport in subports_to_bind | subports_to_unbind:
            if subport in subports_to_bind:
            ...
            # unbind subports
            if subport in subports_to_unbind:
    ...
    def process_ops(self):
        ...
        # Check if the vpp service plugins are enabled
        enable_router_watcher = enable_trunk_watcher = False
        for service_plugin in cfg.CONF.service_plugins:
            if 'vpp-router' in service_plugin:
                enable_router_watcher = True
            elif 'vpp-trunk' in service_plugin:
                enable_trunk_watcher = True
...
class TrunkWatcher(etcdutils.EtcdChangeWatcher):
    ...
    def added(self, parent_port, value):
        ...
        # do we need to wait for the parent port to bind
        props = self.data.vppf.interfaces.get(parent_port, None)
        if props:
            sub_ports = data['sub_ports']
            self.data.bind_unbind_subports(parent_port, sub_ports)
        else:
            # Wait for the parent port to bind
            LOG.debug('awaiting trunk parent port %s to be bound',
                      parent_port)
            self.data.subports_awaiting_parents[parent_port] = data[
                'sub_ports']
    ...
    def removed(self, parent_port):
        ...
        LOG.debug('trunk watcher received unbound for parent port %s ',
                  parent_port)
        subports_to_unbind = self.data.bound_subports[parent_port]
        LOG.debug('trunk watcher unbinding subports %s of parent port %s',
                  subports_to_unbind, parent_port)
        for subport in subports_to_unbind:
            LOG.debug('Unbinding subport %s of parent_port %s',
                      subport, parent_port)
            self.data.vppf.unbind_subport_on_host(subport)
            self.data.bound_subports[parent_port].remove(subport)
