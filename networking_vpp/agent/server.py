...
# ################## BEGIN LISP GPE Methods #################################
    ...
    def ensure_remote_gpe_mapping(self, vni, mac, ip, remote_ip):
        ...
        # VPP crashes if a remote and a local mapping is present for a mac
        ...
        if (mac, vni) not in self.gpe_map['remote_map'] and mac not in \
            ...
            # Add a LISP ARP/NDP entry for remote instance's IPv4/v6 address
            # if it does not exist. If an ARP or NDP entry exists, replace it.
            # A stale entry may exist and must be replaced during re-sync.
            # One possible reason for a stale ARP/NDP entry could
    ...
    def add_local_gpe_mapping(self, vni, mac):
        ...
        # GPE crashes in VPP, if a local and remote EID maps
