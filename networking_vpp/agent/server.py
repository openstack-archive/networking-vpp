    ...
    def clean_interface_from_vpp(self, uuid, props):
        ...
        if props['bind_type'] == 'vhostuser':
            ...
            if props['bind_type'] == 'plugtap':
                self._clean_kernelside_plugtap(uuid,
                                               props['sw_if_index'],
                                               props['ext_tap_name'],
                                               props['int_tap_name'])
        else:
        ...
        for iface in self.vpp.get_interfaces():
            ...
            if port_uuid is not None:
                ...
                if if_index in all_vhostusers:
                    ...
                    if path != all_vhostusers[if_index]['sock_filename']:
                        LOG.warn('Existing interface %s for port %s '
                                 'has a valid UUID but '
                                 'incorrect vhostuser socket file, cleaning',
                                 if_name, port_uuid)
            ...
            def resync_start(self, etcd_results):
                ...
                if self.cold_resync:
                    ...
                    self.cold_sync_networks = \
                        self.vppf.find_and_fix_vpp_networks()
                    self.cold_sync_ports = self.vppf.find_and_fix_vpp_ports()
            ...
            def resync_end(self):
                ...
                if self.cold_sync:
                    ...
                    for uuid, props in self.cold_sync_ports.items():
                        self.vpp.clean_interface_from_vpp(uuid,
                                                          props)
                    ...
                    if action == 'delete':
                        ...
                        if binding_bad:
                            ...
                            if self.cold_sync:
                                ...
                                self.vpp.clean_interface_from_vpp(port,
                                                                  vpp_state)
