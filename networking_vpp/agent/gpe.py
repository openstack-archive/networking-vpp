...
class GpeListener(object):
    ...
    def __init__(self, etcd_listener):
        self.vppf = self.etcd_listener.vppf
        self.gpe = self.vppf.gpe
        self.client_factory = self.etcd_listener.client_factory
        self.host = self.etcd_listener.host
        self.gpe_locators = cfg.CONF.ml2_vpp.gpe_locators
        self.ensure_gpe_dir()
    ...
    def delete_etcd_gpe_remote_mapping(self, segmentation_id, mac_address):
        ...
        gpe_dir = GPE_KEY_SPACE + '/%s/%s' % (segmentation_id,
                                                   self.host)
   ...
   def spawn_watchers(self, pool, heartbeat, data)
            ...
            pool.spawn(gpe.GpeWatcher(self.client_factory.client(),
                                      'gpe_watcher',
                                      GPE_KEY_SPACE,
                                      heartbeat=heartbeat,
