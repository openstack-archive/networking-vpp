...
def unpack_nlhdr(data):
    """Unpack the header of a netlink message

    Returns msg_len, msg_type, flags, seq, pid and the remainder
    of the message
    """

    msg_len, msg_type, flags, seq, pid = struct.unpack("=LHHLL", data[:16])
    if msg_len != len(data):
        LOG.warn('netlink message packet length %d != internal length %d', len(data), msg_len)

    data = data[16:]

    return msg_type, flags, seq, pid, data
...
def unpack_linkmsg(data):
    ...
    """
    if_family, _, if_type, index, flags, change = struct.unpack("=BBHiII", data[:16])
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                # us to resync with either initial state or missed change messages.
                ...
                while True:
                    ...
                    # This is a generic netlink packet header.
                    msg_type, flags, seq, pid, data = unpack_nlhdr(data)
                    ...
                    if msg_type == NLMSG.NOOP:
                        ...
                        raise Exception("Error received on netlink socket")
                    elif msg_type == NLMSG.DONE:
                        # We were presumably resyncing, and now we have everything.

                        # Having processed all of the incoming message, consider
                        # whether we have either new or dead links:
                        LOG.debug('getlink: saw links %s', ', '.join(resync_links))
                    ...
                    # We're interested in tap devices appearing and disappearing.
                    # Anything else can pass us by.
                    ...
                    if link_name is None:
                        raise Exception("Add-link message without if name")
