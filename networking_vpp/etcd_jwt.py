...
class etcd_jwt(object):
        ...
        def _is_path_signed(self, path):
                if (re.match(self.jwt_controller_path_pattern, path)
                   or re.match(self.jwt_computeNode_path_pattern, path)):
                        return(True)
                else:
                        return(False)
        ...
        def _check_NodeName(self, nodeName, vcert_obj):
        ...
        def __init__(self):
                self.jwt_enable = False
                if (cfg.CONF.ml2_vpp.JWT_enable):
                        self.jwt_enable = True

                        self.jwt_controller_path_pattern = \
                            cfg.CONF.ml2_vpp.JWT_controller_path_pattern

                        self.jwt_computeNode_path_pattern = \
                            cfg.CONF.ml2_vpp.JWT_computeNode_path_pattern
        ...
        def verify_jwt(self, signerNodeName, path, value):
                ...
                try:
                        ...
                        if not (self._check_NodeName(signerNodeName,
                                                     vcert_obj)):
                                raise SecurityError()
                        ...
                        LOG.error(("SecurityError : path :%s") % (path))
                        return(False)
                except Exception:
                        LOG.error(("Exception thrown : path :%s") % (path))
                        return(False)
