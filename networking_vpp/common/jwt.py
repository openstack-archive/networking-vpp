...
class JWTUtils(object):
    ...
    def _verify_certificate(self, vcert):
        ...
        raise JWTSigningFailed("Certificate is not trusted")
    ...
    def _check_node_name(self, nodeName, vcert_obj):
        ...
        raise JWTSigningFailed(
            "cert says node name is %s and we want %s"
            % (commonName.value, str(nodeName.value)))
    ...
    def _check_path(self, dval, path):
        ...
        if (dval["_path"] != path):
            raise JWTSigningFailed(
                "path is %s in data and we want %s"
                % (dval["_path"], path)
    ...
    def __init__(self, local_cert, priv_key, ca_cert):
        """Initialize JWTUtils
    ...
    def sign(self, path, value, delta=0):
        ...
        # hashlib.md5(json.dumps(data, sort_keys=True).encode('utf-8')).hexdigest()
    ...
    def verify(self, signer_requirements, path, sgn_value):
        ...
        if sgn_value is None:
            raise JWTSigningFailed("Invalid empty value at path %s" % (path))
        ...
        try:
            ...
            raise JWTSigningFailed("InvalidTokenError: path :%s" % path)
